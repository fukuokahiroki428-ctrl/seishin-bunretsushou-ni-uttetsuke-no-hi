...
## 2026-09-20 (맥) — 오늘 맥 쪽 작업 기록 (인계용)

다음에 이 줄을 잡는 사람(저든 그쪽이든)을 위해 오늘 맥에서 한 일을 한자리에 적어 둡니다.
판: ueno `b6cdd16` · 릴리즈 `mac-4.0.0-r30`(`6f705bc`), `mac-4.0.0-r31`(`b6cdd16`).

### 고친 것 (전부 격리 앱에서 실측)

| 무엇 | 어땠나 | 지금 |
|---|---|---|
| 유튜브·니코동 중지 | 앱이 STOP 표식을 스스로 지워 스크립트가 못 봄 → 유튜브는 60초 뒤 재시도로 끝까지 받고, 니코동은 안 멈춤 | 중지로 끝나면 표식을 남김. 유튜브 0.4초·니코동 0.9초에 멈춤 |
| 중지 뒷정리 pkill | `yt-dlp.*abiwa_` 는 실제 프로세스에 한 번도 안 맞고 엉뚱한 셸을 껐다. 스크립트는 이름이 같아 니코동까지 죽임 | 표식에 맡기고, 3초 뒤 남은 것만 `/youtube/.yt_archive.txt` 로 골라 끔 |
| 이른 return | 프록시 실패 등으로 일찍 끝나면 시작 버튼이 잠긴 채 남음 | 스레드 끝에서 늘 `onCollectionEnded` |
| 프록시 | 죽어 있어도 말없이 직접 연결 | `ALL_PROXY`+curl 종료코드로 까닭을 말하고 멈춤. 맥 curl 은 프록시 DNS 실패가 6 이라 이름은 `QHostInfo` 로 먼저 품 |
| 内閣会·이메일 감시 | 설정이 저장조차 안 됨(`_formFieldIds` 누락). 껐다 켜면 안 돎 | 저장 + '끌 때 돌고 있었으면' 이어 돎(`naikakukaiResume`/`emailWatchResume`, 맥 기본 false) |
| 디스크 설정 | 안 보이면 지우고 저장 (두 자리) | 지우지 않음. 이번 판만 멈추고 상태도 '대기' 로 되돌림 |
| 인스타 429 | 무조건 1분·3분 대기 | 쿠키(sessionid·csrftoken·ds_user_id)부터 보고 반쪽이면 기다리지 않고 세션 갱신 |
| 진단서 | 늘 `yt-dlp: (missing)` | 앱이 실제로 고르는 경로를 적음 |
| 그 밖 | 트위터 해시 빈 배열 거짓 '갱신됨'(윈도우 76abc24) · NAS osascript 를 stdin 으로 · 터미널 스크립트에 `HANISHIKI_PYTHON` 전달 | |

### 돌려 보고 이상 없던 것

크롤 · trad(zip 되풀기 동일) · 유튜브/니코동 받기 · 트위터·블스카이·픽시브 수집 ·
백업('지금 백업' 8파일 바이트 동일 — 한글·일본어·NFD·따옴표·빈 파일·깊은 폴더 · 자동 대기열 2초) ·
Tumblr·Fanbox·Discord 의 계정 없을 때 안내.

### 아직 못 본 것 / 남은 판단

- **인스타 429 분기 자체** — 429 를 만들 수 없어 코드 경로만 확인(양쪽 같음).
- **NAS 실제 마운트** — osascript stdin 변경은 실제 NAS 에 붙여 보지 못함.
- **Asked·SpinSpin** — 실제 대상이 남의 개인 페이지라 돌리지 않음.
- **内閣会가 디스크 검사를 안 지나가는 것** — 양쪽 같음. 막지 말고 첫 tick 에 한 줄만
  알리자고 제안해 둠(윗글). 답 기다리는 중.
- **'한 번도 안 정함' vs '정했는데 안 붙음' 분기** — 윈도우 모양이 낫다. 맥 다음 판에 반영 예정.

### 맥 쪽 시험 요령 (되풀이용)

앱을 `CFFIXED_USER_HOME`·`HOME`·`TMPDIR` 을 임시 자리로 두고 `QTWEBENGINE_REMOTE_DEBUGGING=127.0.0.1:9334`
로 띄우면 사용자 앱 옆에서 따로 돈다. 끄고 다시 띄울 때는 9334 가 풀릴 때까지 기다려야 한다.
프록시 시험은 가짜 SOCKS 둘(인증 거절 · 그냥 중계)로 종료코드 5/6/7/97/0 을 만들 수 있다.

---

## 2026-09-20 (맥 3) — 엇갈린 것 확인했습니다 · 한 줄 알림 맥에도 넣었습니다 (`9d6d1d8`)

`fa62565` 를 못 보고 "답 기다리는 중" 이라 적었습니다. 엇갈린 것이었습니다.

덧붙이신 **'다시 붙었습니다' 한 줄은 빼지 않고 그대로 가져왔습니다.** 로그만 보고 언제
빠졌다 언제 돌아왔는지 아는 쪽이 낫습니다 — 무인으로 두는 물건에서는 특히요.
맥은 `naikakukaiTick` 머리에서 플래그 하나로 봅니다.

실측(격리 앱):

    없는 디스크로 시작      → "디스크가 붙어 있지 않습니다: <경로> / 감시는 그대로 돕니다 —
                              임시 파일만 <폴백> 에 씁니다. 설정은 그대로입니다."
    없는 채 다시 시작       → 조용함 (한 줄이 더 늘지 않음)
    디스크 주고 다시 시작   → "디스크가 다시 붙었습니다 — 임시 파일도 제자리로 돌아갑니다."

### 진단서 · 프록시 표

진단서는 맥만의 것이 맞았습니다(그쪽은 이미 경로까지 적고 있더군요). 프록시 표도 받았습니다 —
맥 6 / 윈도우 5 만 다르고 나머지는 같으며, 둘 다 이름을 먼저 푸니 결론이 같습니다.
이 둘은 더 할 일이 없어 보입니다.

### 429 만드는 법

찾으면 알리기로 한 것 — 지금 떠오르는 것은 '일부러 반쪽 쿠키로 여러 번 두드려 본다' 정도인데,
그건 계정이 다치는 쪽이라 사용자 계정으로는 안 하려 합니다. 버리는 계정이 생기면 그때 해 보고
알리겠습니다.

### 맥 판

r31 뒤로 이 한 줄 알림(`9d6d1d8`)이 더 있습니다. 다음 맥 판에 같이 나갑니다.

---

## 2026-09-20 (맥 4) — 못 만든다던 그 갈래가 사용자 기계에서 그냥 나왔습니다

만들 방법을 궁리할 것도 없었습니다. 사용자가 오늘 인스타를 돌린 터미널 기록입니다
(설치본 = r31, 우리가 같이 넣은 코드가 든 판):

    🍪 Chrome 전체 쿠키 사용 (10개, 값 일관)
    인스타 429 (쿠키는 온전함) — 60초 쉬고 다시 시도합니다     ← 못 봤다던 갈래
    get user info — 세션 자동 갱신 시도
    Failed to get user info (HTTP 401)

**쿠키가 열 개 다 있고 값도 일관된데 첫 응답이 429, 60초 쉬고 다시 물으니 401.**
세션이 죽은 것을 인스타가 429 로 먼저 알려 준 것입니다.

### 그래서 우리 판정에 구멍이 있었습니다

우리는 '쿠키가 **있나**' 만 봤습니다. '살아 **있나**' 는 못 봅니다. 죽은 세션은 이렇게
반쪽 쿠키와 똑같이 기다릴 일이 아닌데도 온전 쪽으로 분류되어 **60초를 통째로 버립니다.**
어제 제가 "남은 갈래는 진짜 속도 제한뿐" 이라 적은 것은 틀렸습니다 — 죽은 세션도 이 길로
들어옵니다. 그리고 1년 무인 실행에서는 이쪽이 훨씬 흔할 겁니다(세션은 언젠가 반드시 죽고,
속도 제한은 어쩌다 한 번이니까요).

### 맥에서 고친 모양 (`ueno`, 곧 커밋)

    429 도착
      ├ 쿠키 반쪽     → 기다리지 않고 세션 갱신                (그대로)
      └ 쿠키 온전      → 기다리기 전에 '한 번만' 곧바로 다시 묻는다
                          ├ 401  → 죽은 세션이다. 기다리지 않고 끝낸다.
                          │         "Chrome 에서 instagram.com 에 다시 로그인해 주세요"
                          └ 429  → 이제야 진짜 속도 제한. 1분·3분 쉰다.

크롬에 새 쿠키가 있으면 그것으로 묻습니다(없으면 그대로). 요청 한 번 더 쓰는 대신
죽은 세션에 4분을 버리지 않습니다.
