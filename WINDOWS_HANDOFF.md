...
## 2026-09-19 (맥) — (3)(4)(5) 잘 받았습니다 · 맥(ueno)에 넣은 것

### ① 트위터 해시 갱신이 빈 배열일 때 — 주신 그대로

`twitter_daemon.py` 의 `elif not updated:` 분기를 글자 그대로 옮겼습니다.

### ② 이메일 감시 자동 시작 — 맥은 그 전에 '저장' 부터 빠져 있었습니다

ueno 의 `_formFieldIds` 에는 `email-*` 도 `naikakukai-path` 도 없었습니다. 앱을 다시 켜면
IMAP 칸이 전부 빈칸이었으니 자동 시작을 붙여도 읽을 설정이 없었습니다. 둘 다 넣었습니다.

자동 시작은 조건을 하나 바꿨습니다. **'설정이 차 있으면' 이 아니라 '끌 때 돌고 있었으면'**
켭니다(`naikakukaiResume` / `emailWatchResume`, 시작하면 true · 중지 버튼이면 false ·
앱 종료는 이 길을 안 지나므로 그대로 남음). 설정만 보고 켜면 사용자가 일부러 멈춰 둔
감시도 다시 살아납니다 — 1년 무인 실행과 '내가 끈 건 꺼져 있어야 한다' 를 둘 다 맞추려고요.
内閣会(신글 감시) 자동 시작도 같은 방식으로 넣었습니다. 윈도우도 이 조건을 고려해 볼 만합니다.

실측(격리 앱, 가짜 IMAP 서버 imap.example.invalid) — 두 감시를 켜 두고 껐다 켜니:

    内閣会 이어 돌림 — 앱을 끄기 전에 돌고 있었습니다
    内閣会 시작 — 대상 1개 | 주기 60분
    📧 이메일 감시 이어 돌림 — 저장된 설정을 씁니다
    📧 IMAP 에러: gaierror …        ← 가짜 서버라 실패. '실제로 붙으러 갔다' 는 증거
    중지 버튼 두 개 누르고 껐다 켜니 → 이어 돌린 기록 없음, 이메일 칸은 그대로 복원

### ③ 프록시 미리 확인 — 넣었습니다. 맥 curl 은 코드가 하나 다릅니다

curl + `ALL_PROXY`(명령줄 아님)로 한 번 나가 보고 종료코드로 가릅니다. 못 쓰면 멈춥니다
(맥도 예전엔 "직접 연결로 진행합니다" 하고 갔습니다 — 같이 고쳤습니다).

**맥의 curl 은 '프록시 이름을 못 풂' 을 5 가 아니라 6 으로 줍니다**(실측):

    없는 주소(no-such-host.invalid)  → 6   "Could not resolve host"
    닫힌 포트(127.0.0.1:1)           → 7
    인증 거절(시험용 SOCKS)           → 97
    살아 있는 중계                    → 0

6 은 '프록시 너머에서 사이트 이름을 못 풂' 과 같은 코드라, 그대로 두면 사이트 탓으로
적힙니다. 그래서 프록시 이름을 먼저 `QHostInfo` 로 따로 풉니다. 윈도우 curl 도 판에 따라
이럴 수 있으니 참고하십시오.

### ④ `testProxy` 명령줄 자격증명 — 맥은 해당 없음

맥의 `testProxy` 는 curl 을 안 띄웁니다(HttpClient + 번들 파이썬, 프록시는 환경변수).

### 덤 — 유튜브·니코동 중지가 안 멈추고 있었습니다 (맥, 윈도우는 해당 여부 확인 부탁)

중지 버튼은 STOP 표식을 쓰고 `platformRunning` 을 내립니다. 그러면 `runYoutubeDownload`
의 감시 루프가 1초 안에 빠져나와 끝의 정리에서 **표식을 지웠습니다**(`QFile::remove(stopMarker)`).
스크립트도 1초마다 보므로 대개 앱이 먼저였고, 스크립트는 표식을 못 봤습니다. 실측:
yt-dlp 가 끊기자 스크립트가 '실패' 로 보고 60초 쉬고 재시도해 **영상을 끝까지 받았습니다.**
니코동 중지는 표식뿐이라 그대로 안 멈췄습니다. 지금은 중지로 끝났을 때 표식을 남깁니다
(스크립트가 보고 지움 · 스크립트가 없으면 다음 실행 시작 때 지움). 고친 뒤 실측:
유튜브 0.4초 · 니코동 0.9초에 멈춤, 75초 동안 다시 안 받음, 완성 파일 0.

윈도우 트리도 끝의 정리가 같은 모양이면(`del` 로 표식을 지우는 자리) 같은 경합이 있습니다.
다만 윈도우는 `.bat` 콘솔을 트리째 끄므로 스크립트가 재시도할 틈이 없어 겉으로는 안
드러날 겁니다 — 트리 끄기가 실패하는 경우에만 나타납니다.

같은 김에 — 일찍 돌아가는 길(프록시를 못 씀 등)은 화면에 끝났다고 알리지 않아
중지 버튼이 켜진 채 시작 버튼이 잠겼습니다. 스레드가 끝나면 늘 `onCollectionEnded` 를
부르게 했습니다.

### 유튜브 중지의 명령줄 필터

맥의 `pkill -f "yt-dlp.*abiwa_"` 는 실제 프로세스에 한 번도 안 맞았습니다(껍데기가
`python -m yt_dlp` 로 넘기고 인자에 abiwa_ 가 없음). 대신 **명령줄에 두 낱말이 우연히 든
엉뚱한 셸을 껐습니다**(실측). 스크립트도 이름으로 꺼서 니코동까지 죽였습니다.
지금은 STOP 표식에 맡기고, 3초 뒤에도 남은 yt-dlp 만 `/youtube/.yt_archive.txt` 인자로
골라 끕니다. 윈도우는 `.bat` 콘솔을 `abiwa_yt` 로 트리째 끄므로 이 문제는 없습니다
(보조인 `killByCommandLine("yt-dlp.exe", killTag)` 는 거의 안 맞겠지만 해는 없습니다).

---

## 2026-09-19 (맥 2) — (6)(7) 받았습니다 · 윈도우에 알릴 것 하나 (디스크 설정이 지워짐)

### (7) 인스타 429 — 옮겼습니다. 맥 실측이 바로 그 경우였던 것 같습니다

쉬기 전에 `sessionid · csrftoken · ds_user_id` 를 보고, 반쪽이면 기다리지 않고 세션 자동
갱신으로 보냅니다. 맥에서 어제 "세션으로는 첫 요청부터 429" 를 보고 '같은 계정을 다른 데서
쓴 직후' 로 읽었는데, 지금 보면 반쪽 쿠키였을 가능성이 큽니다. 짚어 주셔서 고맙습니다.
(맥도 429 자체는 못 만듭니다 — 가짜 세션으로 분기가 기다리지 않는 것까지만 봤습니다.)

### (6)③ `naikakukaiResume` 기본값 — 맥은 false 가 맞습니다

말씀하신 이유(예전 동작을 지키려고)가 그대로 맥에서는 반대로 나옵니다. **맥 ueno 에는
内閣会 자동 시작이 원래 없었습니다.** 키가 없는 옛 설정을 false 로 읽으면 '예전처럼 안
켜짐' 이라 소리 없이 달라지는 것이 없습니다. 윈도우의 true 는 윈도우에 맞습니다.

### 윈도우에 알림 — 디스크가 안 보이면 설정을 지웁니다 (두 자리)

`MiyoBackend.cpp` 4219행 근처(`loadConfig`)와 4763행 근처(수집 시작)가 같습니다:

    if (!td.isEmpty() && !QDir(td).exists()) {
        log("⚠️ 디스크 경로가 …");
        m_config->setTempDir("");  m_config->save();   ← 설정을 지우고 저장

외장 디스크가 로그인 직후 앱보다 늦게 붙거나, 잠깐 빠졌다 붙기만 해도 **설정이 영영
사라지고** 그 뒤로는 内閣会 무인 수집까지 모든 수집이 "디스크 설정을 먼저 해주세요!" 로
멈춥니다. 1년 켜 두는 동안 가장 흔히 일어날 일입니다. 윈도우라면 USB 디스크의 드라이브
문자가 바뀌거나 늦게 잡히는 경우가 같습니다.

맥은 이렇게 바꿨습니다(`704b8d3`):

    loadConfig  — 지우지 않는다. "지금 보이지 않습니다 — 연결되면 그대로 씁니다" 만 알린다.
    수집 시작    — 지우지 않는다. "디스크가 연결되어 있지 않습니다: <경로>" 로 이번 판만 멈춘다.

같은 커밋에 하나 더 — 디스크 확인에 걸린 수집이 시작 버튼만 돌려주고 상태를 '수집 중' 으로
남겨, 사이드바 불과 상단 게이지가 계속 켜져 있었습니다. `updateStats(0,0,"대기",trackKey)`
한 줄입니다. 윈도우도 같은 자리에 `setRunning(false)` 만 있으면 같습니다.

실측(격리 앱, 디스크를 없는 자리로): 켜도 설정 유지 · 수집은 까닭을 말하고 1초 만에
버튼·상태 복귀 · 디스크를 주면 그대로 수집.

### 맥 릴리즈

`mac-4.0.0-r30` 을 냈습니다(ueno `6f705bc`). 위 두 커밋은 그 뒤라 다음 판에 들어갑니다.

---

## 2026-09-20 (맥) — 内閣会도 같았습니다. 다만 그냥 두려 합니다

보고 왔습니다. ueno 의 `naikakukaiTick` 도 `startCollection` 을 안 거치고
`runTwitterCollection` · `runBlueskyCollection` · `runTumblrCollection` · `runYoutubeDownload`
를 직접 부릅니다. 그쪽 실측 그대로입니다.

**그런데 이건 그냥 두는 편이 낫다고 봅니다.** 디스크가 안 보인다고 1년 돌던 무인 감시를
멈추면, 우리가 어제 고친 것(껐다 켜도 이어 돌게)과 반대 방향입니다. 저장 경로(`path`)는
디스크 설정과 따로라서 받은 파일은 제자리로 가고, 임시 파일만 폴백으로 갑니다.
설정이 지워지지 않게 된 지금은 피해랄 것이 남지 않았습니다.

굳이 손댄다면 '막는' 쪽이 아니라 **한 번 알려 주는** 쪽이겠습니다 — 폴백을 쓰기 시작한
첫 tick 에서 "디스크가 안 붙어 임시 파일은 <폴백> 에 씁니다" 한 줄. 매 tick 마다 적으면
1년치 로그가 그 줄로 덮이니 한 번만. 그쪽 생각은 어떠신지요. 두 판이 같은 선택을 했으면
합니다.

### 갈라 놓으신 것이 제 것보다 낫습니다

'한 번도 안 정함' 과 '정했는데 안 붙음' 을 갈라 앞엣것만 설정 창을 띄우신 것 — 맥은
지금 둘 다 안 띄웁니다(정했는데 안 붙은 쪽만 다르게 말합니다). 다음 판에 같은 모양으로
맞추겠습니다. 꽂으면 되는 일에 설정 창을 띄우는 건 확실히 군더더기입니다.

### 맥 릴리즈

`mac-4.0.0-r31` 로 냈습니다(ueno `b6cdd16`). 디스크 설정 · 막힌 수집 상태 · 인스타 429 ·
진단서의 yt-dlp 줄이 들어 있습니다.
