...
public slots:
    ...
    void crawlSiteMirror(const QString &startUrl, int maxPages, bool sameDomain, bool downloadMedia, int maxDepth = 999, bool waitForUser = false);
...
private:
    ...
    // ★ 리소스를 앱이 띄운 Chrome(페이지 컨텍스트 fetch)으로 받아 저장 — curl 은 봇 탐지에 막힘.
    //   실제 브라우저 쿠키/세션/핑거프린트로 요청 → 차단 우회. 크롤 스레드에서 블로킹 호출.
    bool fetchViaChrome(const QString &url, const QString &outPath, int timeoutMs = 40000);

    // ★ 현재 페이지가 Cloudflare/봇 챌린지(캡챠) 페이지인지 감지 — 제목/본문/CF 요소 검사.
    //   true 면 사용자가 직접 풀어야 실제 콘텐츠가 뜸. 크롤 스레드에서 블로킹 호출.
    bool isChallengePage(int timeoutMs = 15000);

    // 크롤러 인스턴스
    PenChromeCrawler *m_crawlChrome = nullptr;
    ...
    // 사이트 미러 — URL → 페이지 제목 (인덱스/오프라인 검색용)
    QMap<QString, QString> m_mirrorTitles;
