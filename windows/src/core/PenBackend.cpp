...
#include "platforms/PenChromeCrawler.h"
...
{
    // ★ MiyoBackend 와 같은 이유 — 페이지가 뜨기 전이면 MainWindow 가 담아 둔다.
...
// ═════════════════════════════════════════════════════════════════════════
// ★ 리소스를 앱이 띄운 Chrome(현재 페이지 컨텍스트의 fetch)으로 받아 저장.
//   curl 은 봇 탐지/Cloudflare 등에 "AI/비브라우저 요청"으로 막혀 HTML 만 오고 자원이 안 옴.
//   페이지 컨텍스트 fetch 는 실제 브라우저 쿠키·세션·핑거프린트를 그대로 써서 차단을 우회한다.
//   (같은 도메인 자원은 CORS 무관. 크롤 스레드에서 세마포어로 블로킹 호출.)
// ═════════════════════════════════════════════════════════════════════════
bool PenBackend::fetchViaChrome(const QString &url, const QString &outPath, int timeoutMs)
{
    if (!m_crawlChrome) return false;
    QString jsUrl = url;
    jsUrl.replace("\\", "\\\\").replace("'", "\\'");
    QString js =
        "(async()=>{try{"
        "const r=await fetch('" + jsUrl + "',{credentials:'include'});"
        "if(!r.ok)return'';"
    ...
    QMetaObject::invokeMethod(this, [this, js, sem, b64]() {
        if (!m_crawlChrome) { sem->release(); return; }
        m_crawlChrome->evaluate(js, [sem, b64](const QJsonValue &v) {
            *b64 = v.toString();
            sem->release();
        });
    }, Qt::QueuedConnection);
    if (!sem->tryAcquire(1, timeoutMs)) return false;
    ...
    out.write(data);
    out.close();
    FileHelper::setDownloadMeta(outPath, url);
    return true;
}

// ═════════════════════════════════════════════════════════════════════════
// ★ 현재 페이지가 Cloudflare/봇 챌린지(캡챠)인지 감지 — 제목/본문 마커 + CF 요소.
// ═════════════════════════════════════════════════════════════════════════
bool PenBackend::isChallengePage(int timeoutMs)
{
    if (!m_crawlChrome) return false;
    // ★ 제목(가장 확실 — 통과하면 실제 페이지 제목으로 바뀜) + 활성 챌린지 컨테이너만 검사.
    //   본문 innerHTML 정규식(turnstile/challenge-platform 등)은 통과 후 잔여 스크립트에 오탐 → 무한루프
    //   → 제거함. 챌린지 컨테이너 ID 들은 통과하면 DOM 에서 사라진다.
    static const QString js =
        "(()=>{try{"
        "const t=(document.title||'').toLowerCase().trim();"
        "if(t.includes('just a moment')||t.includes('잠시만')||t.includes('checking your browser')"
        "||t.includes('attention required')||t.includes('one moment')||t.includes('잠시 후')"
        "||t.includes('잠깐')||t.includes('verify you are human')) return true;"
        "if(document.querySelector('#challenge-running,#cf-challenge-running,#challenge-stage,#cf-please-wait,#trk_jschal_js')) return true;"
        "return false;"
        "}catch(e){return false;}})()";
    auto sem = std::make_shared<QSemaphore>(0);
    auto res = std::make_shared<bool>(false);
    QMetaObject::invokeMethod(this, [this, sem, res]() {
        if (!m_crawlChrome) { sem->release(); return; }
        m_crawlChrome->evaluate(js, [sem, res](const QJsonValue &v) { *res = v.toBool(); sem->release(); });
    }, Qt::QueuedConnection);
    if (!sem->tryAcquire(1, timeoutMs)) return false;
    return *res;
}
...
void PenBackend::crawlStart(const QString &startUrl, const QString &savePath, bool useUserProfile)
{
    ...
    sp.replace("\\", "/");
    while (sp.endsWith("/")) sp.chop(1);                       // ★ "D:/" → "D:" (이후 + "/sub" = "D:/sub") 이중슬래시 방지
    if (sp.length() < 2) sp = QDir::homePath() + "/Downloads/Pen";   // 루트만 남으면 안전망
    ...
    m_crawlChrome = new PenChromeCrawler(this, this);
    ...
    *onChromeStarted = [this, startUrl, onChromeStarted](bool ok, bool isRetry) {
        ...
        m_crawlChrome->navigate(startUrl, [this](bool navOk) {
            ...
            // navigate 실패 메시지는 PenChromeCrawler::navigate 안에서 상세 출력 (중복 X)
        });
    };
...
void PenBackend::crawlAuto(const QString &startUrl, int maxPages, bool sameDomain, bool downloadMedia)
{
    ...
    QThread *t = QThread::create([this, start, maxP, sd, dm]() {
        ...
        while (!queue.isEmpty() && !m_autoCrawlStop.load()) {
            ...
            // ★ Cloudflare/캡챠 — 매 페이지. JS 챌린지("잠시만 기다리십시오")는 자동 통과 폴링(사람 개입 X),
            //   자동으로 안 풀리는 진짜 캡챠만 사용자가 직접 풀 때까지 대기 → 챌린지 페이지 캡쳐 방지.
            if (isChallengePage()) {
                for (int i = 0; i < 12 && !m_autoCrawlStop.load(); ++i) {   // 최대 ~24초 자동 통과 대기
                    QThread::sleep(2);
                    if (!isChallengePage()) break;
                }
                int cfAttempts = 0;
                while (isChallengePage() && !m_autoCrawlStop.load()) {
                    if (++cfAttempts > 3) {
                        QMetaObject::invokeMethod(this, [this]() { log("재확인이 계속됩니다 — 진행 (감지 오탐 가능)", "warning"); }, Qt::QueuedConnection);
                        break;
                    }
                    m_waitingForUser.store(true);
                    QMetaObject::invokeMethod(this, [this]() {
                        log("⏸ Cloudflare/캡챠 — Chrome 창에서 직접 풀고 '로그인 완료 ✓' 를 누르세요 (이후엔 쿠키로 자동 통과)", "warning");
                        runJs("if(window.onCrawlPaused) onCrawlPaused();");
                    }, Qt::QueuedConnection);
                    m_userPauseSem.acquire();
                    m_waitingForUser.store(false);
                    QMetaObject::invokeMethod(this, [this]() { runJs("if(window.onCrawlResumed) onCrawlResumed();"); }, Qt::QueuedConnection);
                    QThread::sleep(2);
                }
            }
...
// ═════════════════════════════════════════════════════════════════════════
void PenBackend::crawlSiteMirror(const QString &startUrl, int maxPages, bool sameDomain, bool downloadMedia, int maxDepth, bool waitForUser)
{
    if (startUrl.isEmpty()) {
    ...
    QThread *t = QThread::create([this, start, maxP, maxD, sd, dm, savePath, waitForUser]() {
        QSet<QString> visited;
        ...
        // 파일/경로 세그먼트 안전화 (Windows 금칙어 + 제어문자)
        ...
        // ★ 제외 URL — github.com/gnuboard/gnuboard5 실제 소스 기준.
        //   액션 스크립트는 항상 /bbs/<script>.php 라서 '/login.php' 처럼 슬래시+.php 로
        //   정확 매칭 → bo_table=login 같은 게시판 이름 오탐을 피한다.
        ...
        while (!queue.isEmpty() && !m_autoCrawlStop.load()) {
            ...
            // ★ Cloudflare/캡챠 — 매 페이지 처리 (이 사이트는 페이지마다 챌린지가 뜸).
            //   (1) Cloudflare JS 챌린지("잠시만 기다리십시오")는 보통 수 초 후 자동 통과 → 폴링 대기
            //       (사람 개입 불필요). cf_clearance 쿠키가 영구 프로필에 쌓이면 점점 덜 뜸.
            //   (2) 자동으로 안 풀리는 진짜 캡챠(Turnstile 등)만 사용자가 직접 풀 때까지 대기.
            if (isChallengePage()) {
                if (m_autoPageCount == 0)
                    QMetaObject::invokeMethod(this, [this]() { log("Cloudflare 확인 중 — 자동 통과 대기...", "info"); }, Qt::QueuedConnection);
                for (int i = 0; i < 12 && !m_autoCrawlStop.load(); ++i) {   // 최대 ~24초 자동 통과 대기
                    QThread::sleep(2);
                    if (!isChallengePage()) break;
                }
            ...
            {
                bool challenge = isChallengePage();
                bool needWait = challenge || (m_autoPageCount == 0 && waitForUser);
                int waitAttempts = 0;
                while (needWait && !m_autoCrawlStop.load()) {
                    if (++waitAttempts > 3) {                // ★ 안전장치 — 감지 오탐으로 무한 대기 방지 (3회 후 진행)
                        QMetaObject::invokeMethod(this, [this]() {
                            log("재확인이 계속됩니다 — 그냥 캡쳐를 진행합니다 (감지 오탐일 수 있음)", "warning");
                        }, Qt::QueuedConnection);
                        break;
                    }
                    m_waitingForUser.store(true);
                    QMetaObject::invokeMethod(this, [this, challenge]() {
                        log(challenge
                            ? "⏸ Cloudflare/캡챠 — Chrome 창에서 직접 풀고 '로그인 완료 ✓' 를 누르세요 (이후 페이지는 쿠키로 자동 통과)"
                            : "⏸ 첫 페이지 확인 — 준비되면 '로그인 완료 ✓' 를 누르세요", "warning");
                        runJs("if(window.onCrawlPaused) onCrawlPaused();");
                    }, Qt::QueuedConnection);
                    m_userPauseSem.acquire();                 // crawlResumeFromUser() 가 release 할 때까지 블록
                    m_waitingForUser.store(false);
                    QMetaObject::invokeMethod(this, [this]() {
                        runJs("if(window.onCrawlResumed) onCrawlResumed();");
                    }, Qt::QueuedConnection);
                    QThread::sleep(2);                        // 통과 후 실제 페이지 렌더/리다이렉트 안정화
                    challenge = isChallengePage();
                    needWait = challenge;                     // 여전히 챌린지면 다시 대기
                    QMetaObject::invokeMethod(this, [this, challenge]() {
                        log(challenge ? "아직 Cloudflare/캡챠 페이지 — 마저 풀고 다시 '로그인 완료 ✓' 누르세요"
                                      : "▶ 통과 확인 — 캡쳐 진행", challenge ? "warning" : "success");
                    }, Qt::QueuedConnection);
                }
            }

            // ★ 첫 페이지에서 사이트 엔진 감지 (GNUBoard / WordPress / XE·Rhymix …)
            //   → UI 표시 + 로그. (감지 결과는 인덱스 그룹핑/배지에 활용)
            if (m_autoPageCount == 0) {
            ...
            // ★ SingleFile lib 로드 — readSingleFileLib() 가 Windows 경로(<exe>/tools/singlefile_extension/lib/single-file.js)
            //   를 포함해 올바르게 읽음. (이전엔 비어있는 멤버 s_singleFileLibCode + mac 전용 폴백만 →
            //   Windows 에서 lib 미로드 → singlefile undefined → getPageData 'no lib' → 캡쳐 조용히 스킵)
            QString libCode = readSingleFileLib();
            QString sfCall = R"JS(
                (async () => {
        ...
        for (auto it = m_mirrorUrlMap.constBegin(); it != m_mirrorUrlMap.constEnd(); ++it) {
            ...
            for (auto m = m_mirrorUrlMap.constBegin(); m != m_mirrorUrlMap.constEnd(); ++m) {
                ...
                //   게시판 쿼리 링크(board.php?bo_table=x&amp;wr_id=y)가 로컬로 연결되려면
                //   양쪽 폼을 모두 치환해야 한다.
                QStringList needles{srcUrl};
                QString amp = srcUrl; amp.replace("&", "&amp;");
                if (amp != srcUrl) needles << amp;
                for (const QString &n : needles) {
                    QString q1 = "\"" + n + "\"", q2 = "'" + n + "'";
                    if (html.contains(q1)) html.replace(q1, "\"" + relPath + "\"");
                    if (html.contains(q2)) html.replace(q2, "'" + relPath + "'");
                }
            }
        ...
        while (!pageQueue.isEmpty() && !m_autoCrawlStop.load()) {
            ...
            *extractResult = QJsonObject();
            ...
            for (const QString &resUrl : urlList) {
                ...
                // ★ curl 대신 앱이 띄운 Chrome(페이지 컨텍스트 fetch)으로 받음 — 봇 차단 우회.
                if (fetchViaChrome(resUrl, resLocal)) {
                    dlOk++;
                } else {
...
void PenBackend::openTerminalLog()
{
    ...
    // ★ 외부 콘솔창(cmd /c start)을 띄우지 않는다 — detached 라 작업관리자에 카메라와 별개의
    //   최상위 '명령 프롬프트'로 뜨고 창도 보였다. 동일한 로그는 이미 앱 내부 로그(appendLog)에
    //   카메라 안에서 표시되므로 외부 콘솔은 불필요. (.pen_log.txt 파일 기록은 그대로 유지)
    (void)scriptPath;
#else
...
void PenBackend::crawlTwitterMirror(const QString &profileUrl, int maxScrolls)
{
    ...
    QThread *t = QThread::create([this, profile, scrolls, mirrorRoot]() {
        ...
        QString libCode = readSingleFileLib();   // ★ Windows 경로(<exe>/tools/...) 포함 로더
        QString sfCall = R"JS(
            (async () => {
                if (typeof singlefile === 'undefined') return {error:'no lib'};
                try {
