...
private:
    ...
    //   그런데 실제로 뮤텍스를 잡는 곳은 몇 군데뿐이고, 수집기들은 워커 스레드에서
    //   맵을 맨손으로 읽고 썼다. 한쪽만 잠그는 자물쇠는 잠그지 않은 것과 같다.
    //   결과는 두 가지였다.
    ...
    //   ※ 맥(HanishikiBackend)이 먼저 이렇게 고쳤다. 같은 구조로 맞춘 것이다.
...
private:
    ...
    //   잠금을 들고 순회하지 않는다 — 키 복사 먼저, 판단은 나중.
    ...
    // platform 과 platform#0, platform#1 … 을 한꺼번에 내린다.
