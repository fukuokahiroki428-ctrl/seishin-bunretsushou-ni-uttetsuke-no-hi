...
def main():
    ...
    def add_exif(filepath, post_data):
        ...
        #   ※ 결과로 하나 더 고쳐졌다: 예전엔 PIL 로 다시 저장(quality=95)해서
        #     받아둔 원본 JPEG 을 다시 압축했다. 지금은 바이트를 그대로 둔다.
        try:
            import exif_tags
            ...
            # ★ 원본을 곱바로 덮지 않는다. 쓰다 실패하면 사진이 깨진 채로 남는다.
    ...
    #   (맥 실측 2026-09-15: 10분에 465파일). 목록마다 '이번에 담은 개수' 로 센다.
    cap_limit = 0
    def over_cap(n):
