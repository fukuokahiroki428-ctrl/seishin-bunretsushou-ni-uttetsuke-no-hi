...
//   예전엔 "❌ HTTP 403 에러" 만 찍고 끝이었다. 그런데 403 은 원인이 여러 가지고
//   (채널을 못 봄 / 권한 없음 / 계정 미인증 / 앞단 차단), 디스코드는 그 이유를
//   언제나 JSON 본문에 담아 보낸다. 그걸 버리고 숫자만 보여 주니 쓰는 사람도
//   고치는 사람도 무엇을 해야 할지 알 수가 없었다.
//   여기서 본문을 읽어 무엇을 해야 하는지까지 알려 준다.
static QString describeDiscordError(const HttpResponse &resp)
{
                                            ...
                                            int limit, const QString &before)
{
    ...
    if (resp.statusCode == 429) {
        ...
        emit m_signals->log(QString("❌ 메시지 가져오기 실패: %1").arg(describeDiscordError(retryResp)));
    } else {
        emit m_signals->log(QString("❌ 메시지 가져오기 실패: %1").arg(describeDiscordError(resp)));
    }
...
QJsonObject DiscordCollector::fetchChannelInfo(const QString &channelId, const QString &token)
{
    ...
    // 예전엔 여기서 조용히 빈 객체만 돌려줬다. 채널 정보를 못 얻으면 이후가 전부
    // 어긋나는데 아무 말이 없어서, 무엇이 잘못됐는지 알 길이 없었다.
    emit m_signals->log(QString("❌ 채널 정보 가져오기 실패: %1").arg(describeDiscordError(resp)));
    return QJsonObject();
}
...
QJsonArray DiscordCollector::fetchGuildChannels(const QString &guildId, const QString &token)
{
    ...
    emit m_signals->log(QString("❌ 서버 채널 목록 가져오기 실패: %1").arg(describeDiscordError(resp)));
    return QJsonArray();
}
