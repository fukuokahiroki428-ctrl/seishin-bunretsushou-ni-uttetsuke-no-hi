...
{
    QString p = platform.isEmpty() ? QStringLiteral("penmirror") : platform;
...
{
    if (!m_config) { log("Config 미초기화", "error", "penmirror"); return; }
    QString url  = m_config->webdavUrl();
    QString user = m_config->webdavUser();
    QString pass = m_config->webdavPass();
    if (url.isEmpty()) {
        log("WebDAV URL 미설정 — 먼저 URL 입력하세요", "warning", "penmirror");
        return;
    }
    log(QString("Finder에 마운트 시도: %1").arg(url), "info", "penmirror");
    ...
    QThread *t = QThread::create([this, script]() {
        ...
        QMetaObject::invokeMethod(this, [this, fin, code, stderrS, newVols]() {
            if (!fin) {
                log("❌ 마운트 타임아웃 (60초)", "error", "penmirror");
                return;
            }
            if (code != 0) {
                QString hint;
                ...
                log(QString("❌ 마운트 실패%1: %2").arg(hint, stderrS.left(150)), "error", "penmirror");
                return;
            }
            if (!newVols.isEmpty()) {
                QString p = "/Volumes/" + newVols.first();
                log(QString("✅ Finder에 마운트됨: %1").arg(p), "success", "penmirror");
                log("   ► 저장 경로를 위 경로로 변경하면 다운로드가 NAS로 직행", "info", "penmirror");
                ...
                log("✅ 마운트 명령 성공 — Finder 사이드바 확인", "success", "penmirror");
            }
        }, Qt::QueuedConnection);
    });
    connect(t, &QThread::finished, t, &QThread::deleteLater);
...
#endif
    log("시스템 설정 → 개인정보 보호 및 보안 → 파일 및 폴더 → Pen 권한 ✅", "info", "penmirror");
}
...
void PenBackend::pickMountedVolume(const QString &targetInputId)
{
...
#endif
    if (items.isEmpty()) {
        log("마운트된 NAS/외장 없음 — WebDAV 카드에서 Finder 마운트 먼저", "warning", "penmirror");
    ...
    QMetaObject::invokeMethod(this, [this, items, paths, targetInputId]() {
        ...
        if (!chosenPath.startsWith(volumeRoot)) {
            log(QString("⚠ 선택한 폴더가 %1 안에 없음 — 무시").arg(volumeRoot), "warning", "penmirror");
            return;
        }
        QDir().mkpath(chosenPath);
        log(QString("[저장경로] 선택: %1").arg(chosenPath), "success", "penmirror");
        QString js = QString(
