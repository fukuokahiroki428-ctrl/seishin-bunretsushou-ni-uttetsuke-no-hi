...
//   findChromeExecutable() 은 Chrome 이 없으면 Edge 를, 그것도 없으면 Brave 를 쓴다.
//   chrome.exe 만 보면 Edge 만 깔린 기계에서 캡쳐 브라우저가 한 번도 정리되지
//   않아 프로세스와 프로필 폴더가 세션마다 쌓인다.
//   맥은 pkill -f 라 이름을 안 보고 잡아서 이 문제가 없다 — 윈도우로 옮기며 생긴 회귀다.
...
// ═════════════════════════════════════════════════════════════════════════
// ═════════════════════════════════════════════════════════════════════════
// 원격 백업(rclone) — WebDAV/HTTPS · FTP · SFTP · S3, 이어올리기(resume)
// ═════════════════════════════════════════════════════════════════════════
//   runRcloneBackup 은 type=webdav 고정 + NAS 마운트 전용이었다.
//   이쪽은 사용자가 종류/주소/자격증명을 직접 넣어 URL 로 바로 올린다(마운트 불필요).
//   rclone copy 는 기본이 skip-existing(같은 크기·시각 건너뜀) + 끊긴 전송 재개라
//   "이어서 올리기" 가 내장이다.
//
...
void HanishikiBackend::startRemoteBackup(const QString &configJson)
{
    ...
    // rclone 탐색 — 맥은 번들 Resources/tools, 윈도우는 exe 옆.
    QString rclonePath = QCoreApplication::applicationDirPath() + "/../Resources/tools/rclone";
#ifdef Q_OS_WIN
    rclonePath = QCoreApplication::applicationDirPath() + "/rclone.exe";
#else
    if (!QFile::exists(rclonePath))
        rclonePath = QCoreApplication::applicationDirPath() + "/../../../resources/tools/rclone";  // 개발 트리
    if (!QFile::exists(rclonePath))
        rclonePath = QCoreApplication::applicationDirPath() + "/rclone";
#endif
    if (!QFile::exists(rclonePath)) {
...
void HanishikiBackend::emailWatchTick()
{
    ...
    // Python script 경로 — AI override 우선(있으면) → 번들
...
#else
    // ★ 번들 도구는 두 곳에 나뉘어 있다 — ffmpeg/ffprobe/deno 는 Contents/MacOS,
    //   yt-dlp 와 rclone 은 Contents/Resources/tools. 여기서 MacOS 만 보는 바람에
    //   yt-dlp 는 늘 못 찾고 아래 폴백으로 떨어져 '시스템(홈브루) yt-dlp' 가 쓰였다.
    //   홈브루가 없는 맥에서는 유튜브 분석·다운로드가 통째로 실패한다.
    QStringList cand;
    cand << appDir + "/" + name
         << Common::bundledToolsDir() + "/" + name
         << Common::bundledToolsDir() + "/" + name + "/" + name;  // exiftool 처럼 폴더 안에 든 경우
                            ...
                            *fetchNext = [this, chromePtr, outFile, totalSize, offsetPtr, fetchNext, filePath, writeTarget, useLocalTemp, url, done, resultOk, trackKey]() {
                                if (*offsetPtr >= totalSize) {
                                    ...
                                    *resultOk = finalOk;
                                    ...
                                    //   그러면 병렬 트랙 전부가 카운터 하나("")를 나누어 쓰게 되어,
                                    //   25회 제한에 트랙 수만큼 빨리 걸린다 — 자기 몫을 다 쓰지도
                                    //   않은 트랙의 Chrome 이 수집 도중 재시작된다.
                                        ...
                                        const std::atomic<bool> *running)
{
    ...
    //   (이전엔 ffmpeg 없는 번들 MacOS/ 를 무조건 가리켜 "ffmpeg could not be found" 로 실패했음)
    for (const QString &ff : QStringList{ appDir + "/ffmpeg",
...
void HanishikiBackend::getSystemInfo()
{
    QThread *thread = QThread::create([this]() {
        ...
        QStringList info;
        info << "═══════════════════════════════════";
        // ★ 옛 이름(カメラ)이 그대로 남아 있었다 — 앱 이름이 네 번 바뀌는 동안 여기만
...
void HanishikiBackend::startLocalLlm(const QString &modelHint)
{
    ...
    if (!QFile::exists(server)) {
        ...
        log(QString("❌ AI 엔진이 설치돼 있지 않습니다.\n"
                    "   설치 위치: %1\n"
                    "   내려받은 DMG 안의 'AI 설치.command' 를 더블클릭하면 엔진과 모델을 받습니다.\n"
        ...
        for (int i = 0; i < kMaxTicks; ++i) {
            ...
            if (h.get(llmBase() + "/v1/models", llmHeaders()).isOk()) {
                ...
                QMetaObject::invokeMethod(this, [this, sec]() {
                    log(QString("✅ 로컬 AI 준비 완료 (%1초, %2) — 자가진단이 이 AI 를 사용합니다.")
                            .arg(sec).arg(llmBase()),
                        "success", "settings");
                    getLlmStatus();
                }, Qt::QueuedConnection);
...
//   ① 쓰기가능 override 에만 쓰고(번들 서명 보호) ② 적용 전 백업 ③ python
//   -m py_compile 문법검증 ④ 실패 시 자동 원복 ⑤ 원클릭 초기화(revert).
...
// ── 읽기 전용 웹 검색 (Brave Search API) — 키가 있어야 동작. AI 가 인터넷으로 진단/답변할 때 사용.
//    반환: 상위 결과 제목+요약 텍스트 블록(없으면 빈 문자열). 명령 실행/링크 방문은 하지 않는다(읽기 전용).
