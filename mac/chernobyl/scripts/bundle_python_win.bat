...
if exist "%MARKER%" (
    "%PYTHON_DIR%\python.exe" -c "import twikit; import httpx; import atproto" 2>nul
    if !errorlevel!==0 (
...
if not exist "%TARBALL%" (
    ...
    if !errorlevel! neq 0 (
        echo [bundle_python_win] Download failed!
        exit /b 1
    )
)
...
if not exist "%PYTHON_DIR%\python.exe" (
    echo [bundle_python_win] ERROR: python.exe not found after extraction
...
echo [bundle_python_win] Installing twikit, httpx, atproto, openpyxl, Pillow, piexif, bs4, websockets, lxml, m3u8, cryptography...
"%PYTHON_DIR%\python.exe" -m pip install --quiet --no-cache-dir twikit httpx atproto openpyxl Pillow piexif beautifulsoup4 websockets lxml m3u8 cryptography 2>&1

REM Verify packages
echo [bundle_python_win] Verifying...
"%PYTHON_DIR%\python.exe" -c "import twikit, httpx, atproto, openpyxl, PIL, piexif, bs4; print('All packages verified!')"
if !errorlevel! neq 0 (
    echo [bundle_python_win] Package verification failed!
    exit /b 1
)
