...
#   앱은 app.setApplicationName(APP_NAME_ASCII) 로 데이터 폴더를 정한다. 즉 ASCII 다.
#   .app 파일명은 Finder 표시용이라 카타카나(ハンイシキ.app)인데, 그걸 그대로 쓰면
#   모델을 Miyo/ハンイシキ/llm 에 넣게 되고 앱은 Miyo/Hanishiki 를 봐서 못 찾는다.
