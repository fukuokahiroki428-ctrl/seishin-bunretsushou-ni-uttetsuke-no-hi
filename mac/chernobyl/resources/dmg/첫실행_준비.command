...
# ★ 앱 이름을 손으로 적지 않는다.
#   이 파일은 DMG 안에 앱과 나란히 들어 있다. 그러니 '옆에 있는 .app' 이 곧 그 앱이다.
#   예전엔 "Hanishiki.app" 이라고 박아 두었는데, Finder 표시를 카타카나로 바꾸느라
#   번들 이름이 ハンイシキ.app 이 되자 바로 이 스크립트가 앱을 못 찾게 됐다.
#   이름이 또 바뀌어도 안 깨지도록 옆에서 찾아 쓴다.
APP_NAME=""
for _c in "$SELF_DIR"/*.app; do
    [ -d "$_c" ] && { APP_NAME="$(basename "$_c")"; break; }
done
[ -z "$APP_NAME" ] && APP_NAME="ハンイシキ.app"   # DMG 밖에서 실행됐을 때의 마지막 보루
