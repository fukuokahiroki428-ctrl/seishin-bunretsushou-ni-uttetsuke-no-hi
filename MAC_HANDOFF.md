...
## 0-5. 병렬 크래시 — 스레드 검증까지 마쳤습니다 (런타임 재현은 여전히 안 됨)

`f114172` 의 널 가드가 **실제로 유효한지**는 "검사와 역참조 사이에 다른 스레드가
끼어들 수 있는가" 에 달려 있습니다. 널 가드는 전형적인 TOCTOU 함정이 있는 패턴이라,
가드를 넣었다고 끝난 게 아닙니다. 그래서 스레드 소속을 따라가 봤습니다.

```
Chrome 생성    invokeMethod(this,…)                    → MiyoBackend 스레드 = 메인
QWebSocket     new QWebSocket(…, this=RealChromeCrawler) → 부모가 메인 → 신호도 메인
CDP 콜백       onWsTextMessage(메인) 에서 m_pendingCmds 호출 → 메인
강제 호출      disconnected 핸들러(메인) 에서 대기 콜백 전부 → 메인
슬롯 비우기    invokeMethod(this,…, QueuedConnection)   → 메인
moveToThread   MiyoBackend 0곳 · RealChromeCrawler 0곳
```

**전부 메인 스레드 한 곳입니다.** 널 검사와 역참조 사이에 슬롯이 비워질 수 없습니다.
`captureRealPageCDP` 자체는 수집 워커에서 돌지만, chromePtr 를 만지는 부분은 전부
invokeMethod 로 메인에 넘기거나 메인에서 오는 콜백 안입니다.

즉 **이 수정은 논리적으로 닫혀 있습니다.** 남은 것은 실제로 그 경로를 밟아 보는 것뿐인데,
계정이 있어야 해서 양쪽 다 못 하고 있습니다.

### 사용자에게 안내한 것

v3.9.7 에는 이 수정이 없으므로 **지금 병렬 수집을 시험하지 말라**고 했습니다.
v3.9.8 이 나온 뒤에 하는 것이 맞습니다.

## 0-6. 맥은 독자 노선으로 갑니다

사용자 지시로 맥은 `ueno` 브랜치에서 따로 갑니다. `main` 은 그대로 쓰시면 됩니다.

```
앱 이름   ハンイシキ  (파일명은 ASCII "Hanishiki")
판        上野        (내부 버전 4.0.0)
브랜치    ueno
```

**windows/ 트리는 더 이상 건드리지 않겠습니다.** 지금까지 양쪽에 넣은 것(널 가드,
killByCommandLine, SFTP, AI 온라인, API 교체 패널, 보관함 등)은 이미 main 에 있습니다.

맥 쪽에서 새로 나온 것 중 윈도우에도 쓸모 있을 만한 것은 여기에 계속 적겠습니다.
가져다 쓰실지는 그쪽 판단에 맡깁니다. 이번 판에서 나온 것:

- **앱 이름이 바뀌면 사용자 데이터가 고아가 된다** — AppDataLocation 이
  `.../Miyo/<앱이름>` 이라, 이름을 바꾸면 AI 모델(9GB)·python_env·크롬 프로필·색인
  DB 가 통째로 남겨집니다. 옛 이름을 코드에 박는 방식은 다음 변경 때 또 깨집니다.
  형제 폴더 중 '우리 흔적이 있고 가장 최근에 쓰던 것' 을 찾아 rename 으로 이어받게
  했습니다. 같은 디스크라 9.8GB 도 즉시 끝납니다(실측).
- **앱 이름이 코드 곳곳에 흩어져 있었다** — main.cpp·백엔드·설치기에 각각
  "Predormition" 이 박혀 있어서, 이름을 바꾸면 일부만 따라오고 나머지는 옛 이름을
  가리켰습니다(데이터 폴더가 갈라지는 원인). CMake 가 한 곳에서 넘기게 했습니다.
